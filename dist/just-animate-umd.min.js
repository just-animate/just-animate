...
function n(t){return a(function(t){return t*t*((e+1)*t-e)},t)}function p(t){return t}var c=19,h=function(t,e,n){var r=1-n
return(3*(r*t+n*e)*r+n*n)*n}
var o=/^\s*[\(\),\/\s]\s*/,s=/^\(/,f=/^\)/,l=/^#[a-f\d]{3,6}/i,v=/^[a-z][a-z\d\-]*/i,y=/^\-?\d*\.?\d+/,d=/^\-?\d*\.?\d+[a-z%]+/i,g=/^[mhvlcsqt]/i,x=1,m=2,b=4,S=x|b,_=8|m,M=16|m,k=32
...
t.pattern=t.pattern.substring(0,t.pos+1+r.length)+",1"+t.pattern.substring(t.pos+1+r.length),t.match="rgba"}return k}if(!t.isPath&&w(t,d))return S
if(w(t,y))return x
...
function z(t){var e
P(T,t)
for(var n,r=p,i=!1,a=[];1;){if(!(e=O(T))){r=A(r,n,a)
...
P($,t)
var r=0,i=""
for(;1&&(e=w(a=$,o)?m:w(a,y)?x:w(a,d)?S:w(a,v)?k:void 0);)e===k&&(n=$.match.toLowerCase(),r=0),e===S||e===x?("matrix"===n?i+=r%3?"0":"1":"matrix3d"===n?i+=r%5?"0":"1":/scale([xyz]|3d)?/i.test(n)?i+="1":i+="0",r++):i+=$.match
var a
...
u?(g=+o,m=+s,b=n,f=Math.round(Math.sqrt(Math.min(Math.max(0,(g*g+m*m)*b),255*255))),u--):f=nt(+o,+s,n),a+=f}else if(r===S||i===S){var p=W.exec(r===S?o:s),c=p[1],h="px"===c,l=parseFloat(o),v=parseFloat(s),f=nt(l,v,n,h)
a+=f+c}else{var y=n<.5?o:s,d=r===k&&i===k&&("rgb"===y||"rgba"===y)
d&&(u=3),a+=y}}var g,m,b
...
var ft="__just_cache"
function pt(t,e,n){if(e[ft]&&e[ft][t])return e[ft][t][n]}var ct,ht=1e3/60,lt=[]
function vt(t){var e=R(t-ct,0,2*ht)
ct=t
for(var n=lt.slice(),r=0,i=n;r<i.length;r++){mt(e,g=i[r])}for(var a=0,u=n;a<u.length;a++){gt(g=u[a])}for(var o=[],s=0,f=n;s<f.length;s++){for(var p=0,c=(g=f[s]).events;p<c.length;p++){var h=c[p]
if(g.listeners){var l=g.listeners[h]
l&&l.length&&Array.prototype.push.apply(o,l)}}g.events.length=0}for(var v=[],y=0,d=n;y<d.length;y++){var g
yt(g=d[y],v)}for(var m=lt.length-1;-1<m;m--)"running"!==n[m].playState&&lt.splice(m,1)
...
for(var r in t.keyframes){var i=t.keyframes[r],a=dt(t,r),u=function(M){for(var t,k=i[M],P=(t=k,Object.keys(t).map(B).sort(I)),e=function(t){var e,n,r,i,a,u,o,s,f=(n=M,"string"==typeof(e=t).tagName&&e.style?0===n.indexOf("--")?it:-1!==ot.indexOf(n)?at:-1!==st.indexOf(n)?rt:ut:rt),p=(r=f)===at?N:r===it?D:r===ut?K:U
if("idle"!==T){var c=Z,h=((s=f)===at?H:s===it?V:s===ut?J:Q)(t,M),l=G(P,O),v=-1===l?0:P[l],y=k[P[l]],d=Math.min(l+1,P.length-1),g=P[d],m=k[g].value,b=z(k[g].ease||"linear"),x=void 0
if(y)x=y.value
else null==(_=pt(A,t,M))&&(i=A,u=M,o=_=h,(a=t)[ft]||(a[ft]={}),a[ft][i]||(a[ft][i]={}),a[ft][i][u]=o),x=_
var S=c(x,m,b((v-O)/(v-g)))
h!==S&&w.push(function(){return p(t,M,S)})}else{var _
null!=(_=pt(A,t,M))&&p(t,M,_)}},n=0,r=a;n<r.length;n++){e(r[n])}}
for(var o in i)u(o)
for(var s=0,f=a;s<f.length;s++){var p=f[s]
"idle"===T&&(e=A,(n=p)[ft]&&n[ft][e]&&delete n[ft][e])}}}function dt(t,e){if(!e)return[]
if(0!==e.indexOf("@"))return Array.prototype.slice.call(document.querySelectorAll(e))
var n=t.targets[e]
if(!n)throw Error("Target "+e+" not configured.")
return"number"==typeof n.length?n:[n]}function gt(t){if("cancel"===t.playState)t.playState="idle"
...
return this.update(),this},e.prototype.delay=function(t,e){return this.animate("",t,{"":0},e)},e.prototype.finish=function(){return this.playState="finish",this.events.push("finish"),this.update()},e.prototype.getConfig=function(){var t={}
...
return t=this,lt.length||(ct=performance.now(),q(vt)),-1===lt.indexOf(t)&&lt.push(t),this},e}()
var St=2*Math.PI
...
var n},u["cubic-bezier"]=function(u,o,s,f){return o=+o,s=+s,f=+f,(u=+u)<0||1<u||s<0||1<s?p:function(t){if(t<=0||1<=t)return t
var e,n,r=0,i=1,a=c
