...
import { types, waapi } from '../core/types';
...
use(effect => {
    ...
    return keys(effect.props).map(propertyName => {
        ...
        for (let i = 0, tLen = times.length; i < tLen; i++) {
            const time = times[i];
            const timeConfig = config[time] as types.IValueJSON;
            ...
            if (timeConfig.type === 'set') {
                ...
                const lastFrame =
                    lastIndex > -1 &&
                    (config[times[lastIndex]] as types.IValueJSON);
                if (lastFrame) {
                    keyframes.push({
            ...
            const nextFrame =
                nextIndex < tLen &&
                (config[times[nextIndex]] as types.IValueJSON);
        ...
        return {
            ...
            updated(ctx: types.IAnimationUpdateContext) {
                const time = ctx.time;
                const rate = ctx.rate;
