import { isArrayLike, isDefined, isString, isNumber, isFunction, isOwner, isDOM } from './inspect';
import { push, all, mapFlatten } from './arrays';
...
function assignRef<T>(refs: any, target: T): T | string {
  for (var ref in refs) {
    if (refs[ref] === target) {
      return ref
    }
...
export function replaceWithRefs<T>(refs: any, target: T, recurseObjects: boolean): T | string {
...
export function resolveRefs(refs: any, value: any, recurseObjects: boolean): any {
