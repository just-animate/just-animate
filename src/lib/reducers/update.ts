...
export const update: IReducer = (model: ITimelineModel, _data: any, ctx: IReducerContext) => {
  ...
  const playing = model.timing.playing
  const time = model.timing.time

  // remove tick from loop if no timelines are playing
  if (!playing) {
    loopOff(model.id)
  }
  ...
  all(model.players, player => {
    ...
    const isActive = playing && inRange(flr(time), from, to)
