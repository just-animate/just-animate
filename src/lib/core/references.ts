...
export function replaceWithRefs<T>(
  refs: References,
  target: T,
  ...
  if (isArrayLike(target)) {
    return mapFlatten(target as any, (t: any) =>
      replaceWithRefs(refs, t, recurseObjects)
    ) as any;
  }
  ...
  if (recurseObjects) {
    for (var name in target as {}) {
      if (isOwner(target, name)) {
        target[name] = replaceWithRefs(
          refs,
          target[name],
          recurseObjects && name !== 'targets'
        );
      }
    }
    return target;
...
export function resolveRefs(
  ...
  if (isArrayLike(value)) {
    const results: any[] = [];
    all(value as any[], item =>
      push(results, resolveRefs(refs, item, recurseObjects))
    );
    return results;
  }
