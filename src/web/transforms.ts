    ...
    for (const transform in effect.values) {
      ...
      if (startingPosFound && endingPosFound) {
        ...
        for (let g = startingPos + 1; g < endingPos; g++) {
          ...
          const offsetDelta =
            (currentOffset - startEffect.offset) /
            (endEffect.offset - startEffect.offset);
          const currentValue =
            startVal.value + (endVal.value - startVal.value) * offsetDelta;

          const currentValueWithUnit =
            currentValue + (endVal.unit || startVal.unit || '');
