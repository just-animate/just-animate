...
describe('splitText()', () => {
    ...
    it('returns the same words and elements if splitText is called twice on the same element', () => {
        ...
        assert.equal(result1.characters.length, result2.characters.length)
        assert.equal(result1.characters[0].textContent, result2.characters[0].textContent)
        assert.equal(result1.characters[1].textContent, result2.characters[1].textContent)
    })
})
