...
import { resolveProperty } from '../../src/lib/resolve-property'
...
describe('resolve', () => {
  ...
  it('resolves the index in functions', () => {
    const actual = resolveProperty((_target: any, index: number) => 'The number is ' + index, {}, 2, 3)
    assert.equal(actual, 'The number is 2')
  })

  it('resolves the target in functions', () => {
    const actual = resolveProperty((target: any) => 'The number is ' + target.number, { number: 3 }, 0, 3)
